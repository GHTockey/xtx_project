<template>
    <div class="sub-sort">
        <div class="sort">
            <a @click="updateSortParams({ sortField: '' })" :class="{ active: sortParams.sortField === '' }">默认排序</a>
            <a @click="updateSortParams({ sortField: 'publishTime' })"
                :class="{ active: sortParams.sortField === 'publishTime' }">最新商品</a>
            <a @click="updateSortParams({ sortField: 'orderNum' })"
                :class="{ active: sortParams.sortField === 'orderNum' }">最高人气</a>
            <a href="javascript:" @click="updateSortParams({ sortField: 'evaluateNum' })"
                :class="{ active: sortParams.sortField === 'evaluateNum' }">评论最多</a>
            <a @click="updateSortParams({ sortField: 'price' })">
                价格排序
        ...
        <div class="check">
            <XtxCheckbox :checked="sortParams.inventory" @onChange="updateSortParams({ inventory: $event })">仅显示有货商品
            </XtxCheckbox>
            <XtxCheckbox :checked="sortParams.onlyDiscount" @onChange="updateSortParams({ onlyDiscount: $event })">仅显示特惠商品
            </XtxCheckbox>
        </div>
    </div>
</template>
...
function updateSortParams(target: Partial<SortParams>) {
    ...
    // 如果用户点击的是价格
    if (sortParams.value.sortField === "price") {
        sortParams.value = {
            ...sortParams.value,
        ...
        sortParams.value.sortField === "publishTime" ||
        sortParams.value.sortField === "orderNum" ||
        sortParams.value.sortField === "evaluateNum"
    ) {
        sortParams.value = {
            ...sortParams.value,
            sortMethod: "",
